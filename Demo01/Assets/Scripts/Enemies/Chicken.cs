...
public class Chicken : MonoBehaviour
{
    ...
    {
        Idle, Walk, Attack, Disappear, Death, Throw
    }
    ...
    void Update()
    {
        if (nextMove != 0)
        {
            lastMoveDirection = new Vector3(nextMove, 0, 0);
        }
        Move();
    ...
    private void SetCurrentAnimation(AnimState _State)
    {
        switch (_State)
        {
            ...
            case AnimState.Throw:
                _AsncAnimation(AnimClip[(int)AnimState.Throw], false, 1f);
                break;
        }
    }
    ...
    void Move()
    {
        if ((_AnimState != AnimState.Attack) && (Died == false) && (isHitting == false) && (_AnimState != AnimState.Throw))
        {
